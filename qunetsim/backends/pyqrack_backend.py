...
try:
    ...
    raise RuntimeError(
        'To use PyQrack as a backend, you need to first install the Python package '
        '\'pyqrack\' (e.g. run \'pip install pyqrack\'.')


class PyQrackBackend(object):
    def __init__(self):
        self._hosts = PyQrackBackend.Hosts.get_instance()
        self._entaglement_pairs = PyQrackBackend.EntanglementPairs.get_instance()
    ...
    class EntanglementPairs(SafeDict):
        ...
        @staticmethod
        def get_instance():
            if PyQrackBackend.EntanglementPairs.__instance is not None:
                return PyQrackBackend.EntanglementPairs.__instance
            else:
                return PyQrackBackend.EntanglementPairs()

        def __init__(self):
            if PyQrackBackend.EntanglementPairs.__instance is not None:
                raise Exception("Call get instance to get this class!")
            PyQrackBackend.EntanglementPairs.__instance = self
    ...
    class Hosts(SafeDict):
        ...
        @staticmethod
        def get_instance():
            if PyQrackBackend.Hosts.__instance is not None:
                return PyQrackBackend.Hosts.__instance
            else:
                return PyQrackBackend.Hosts()

        def __init__(self):
            if PyQrackBackend.Hosts.__instance is not None:
                raise Exception("Call get instance to get this class!")
            PyQrackBackend.Hosts.__instance = self
            SafeDict.__init__(self)
    ...
    def rx(self, qubit, phi):
        ...
        """
        self.engine.r(Pauli.PauliX, phi, qubit.qubit)
    ...
    def ry(self, qubit, phi):
        ...
        Args:
            ...
            phi (float): Amount of roation in Rad.
        """
        self.engine.r(Pauli.PauliY, phi, qubit.qubit)
    ...
    def rz(self, qubit, phi):
        ...
        Args:
            ...
            phi (float): Amount of roation in Rad.
        """
        self.engine.r(Pauli.PauliZ, phi, qubit.qubit)
    ...
    def cnot(self, control, target):
        ...
        """
        self.engine.mcx([control.qubit], target.qubit)
    ...
    def cphase(self, control, target):
        ...
        """
        self.engine.mcz([control.qubit], target.qubit)
    ...
    def release(self, qubit):
        ...
        """
        self.engine.release(qubit.qubit)
